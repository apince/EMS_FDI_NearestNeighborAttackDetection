...
if system == 'p'
    ...
    ii = find(mpc.bus(:,3)~=0 | mpc.bus(:,4)~=0);
    P_loads(ii) = P_load;
    ind_PA = find(mpc.bus(:,3)==0 & mpc.bus(:,4)~=0);
    P_loads(ind_PA) = -999;
    
    if length(P_load) ~= length(mpc.bus(:,1))
        loadsRT = P_loads(P_loads~=-999 & P_loads~=0);
       %loadsRT = P_loads(P_loads~=-999)';
    else
    ...
    load results15
    load graph_texas
    mpc = case_ACTIVSg2000_ASU01;
    %mpc = ext2int(mpc);
    normalLoads = loadsTexas;
    if length(P_load) ~= length(mpc.bus(:,1))
...
if realTimeTesting == 1
    ...
    attackLoadsGraphing(g,ind,groups,TH_out,loadsRT,inddist) 
